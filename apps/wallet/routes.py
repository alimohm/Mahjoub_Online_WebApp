...
def overview():
    ...
    try:
        ...
        # ملاحظة حوكمية: إذا لم تكن علاقة joinedload مع المورد ناضجة بعد، يفضل جلب السجلات بشكل مرن
        ...
        # تجميع المؤشرات المالية في قاموس منظم ومضمون القيمة لمنع أخطاء Jinja2
        platform_metrics = {
        ...
        # حماية إضافية: إذا قوالب الخطأ غير متوفرة، ارمي نصاً صريحاً بدلاً من تجميد السيرفر
...
def approve_withdrawal():
    ...
    try:
        # استخدام with_for_update لمنع الـ Race Condition على سيرفر Railway الحي
        ...
        # الخصم المالي حسب نوع عملة الحوالة المبرمة
        if currency == 'YER':
...
def reject_withdrawal():
    """
    رفض طلب السحب سيادياً: إعادة الأموال المحتجزة إلى رصيد المورد المتاح.
    """
    tx_id = request.form.get('tx_id')
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"status": "success", "message": "تم رفض طلب السحب بنجاح."}), 200
