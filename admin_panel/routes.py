...
# --- 1. استيراد النماذج السيادية (مع معالجة الأخطاء) ---
try:
    from core.models.user import User
    from core.models.vendor import Vendor, WithdrawRequest
    # محاولة استيراد النماذج الإضافية إذا وجدت
    try:
...
def get_sovereign_identity():
    ...
    try:
        # إلغاء أي جلسة معلقة لضمان قراءة أحدث بيانات
        db.session.rollback()
...
def force_repair():
    ...
    try:
        ...
        # 1. تحديث جدول الموردين للسماح بكلمات مرور فارغة (نعتمد على جدول المستخدمين)
        db.session.execute(text("ALTER TABLE vendors ALTER COLUMN password DROP NOT NULL;"))
        
        # 2. ضمان وجود الأعمدة السيادية الجديدة
...
@login_required
def admin_dashboard():
    # منع غير المشرفين من دخول الإدارة
    if getattr(current_user, 'role', '') != 'admin':
    ...
    try:
        ...
        db.session.rollback()
        # إرجاع قيم صفرية بدلاً من انهيار الصفحة في حال وجود خطأ بقاعدة البيانات
...
def add_supplier():
    if request.method == 'POST':
        try:
            ...
            # أ: إنشاء الكيان في جدول المستخدمين (User)
            new_user = User(
                username=username,
            ...
            db.session.flush() # للحصول على user.id قبل الـ commit

            # ب: إنشاء الكيان في جدول الموردين (Vendor)
            new_vendor = Vendor(
                user_id=new_user.id,
...
def manage_suppliers():
    try:
        # جلب الموردين مع بيانات حساباتهم (User) لتقليل استعلامات DB
        suppliers_list = db.session.query(Vendor).options(joinedload(Vendor.user)).all() if Vendor else []
...
def manage_wallets():
    try:
        # عرض أرصدة الموردين وحالة المحافظ
...
def withdraw_requests():
    try:
        # عرض طلبات سحب الأموال من الأحدث للأقدم
...
def login():
    # إذا كان مسجلاً للدخول كمسؤول، وجهه للوحة التحكم فوراً
