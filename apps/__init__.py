...
def create_app():
    ...
    # 1. سياق النماذج والتطهير السيادي لقاعدة البيانات
    with app.app_context():
        ...
        try:
            db.create_all()
            # أوامر الـ SQL الخاصة بك كما هي تماماً
            ...
            db.session.rollback()
            print(f"❌ تحذير في بناء الهيكل: {e}")
        finally:
            db.session.close()
    ...
    # 3. الاستيراد الآمن للمحركات (السر هنا)
