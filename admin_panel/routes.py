...
# --- 1. مسار الطوارئ السيادي (ترميم شامل للأرصدة والهيكل) ---
@admin_bp.route('/force-repair-now')
def force_repair():
    try:
        db.session.rollback() 
        # ترميم جداول المستخدمين
        ...
        # ترميم جداول الموردين وإضافة الأعمدة المالية السيادية
        ...
        # إضافة أعمدة الأرصدة (حل مشكلة الصورة image_f08053.png)
        ...
        flash("تم ترميم الترسانة الرقمية وإضافة كافة حقول الأرصدة بنجاح.", "success")
...
def add_supplier():
    if request.method == 'POST':
        try:
            ...
            # 2. إنشاء بيانات المورد مع تصفير الأرصدة لضمان التوافق
            new_vendor = Vendor(
                user_id=new_user.id,
...
def manage_wallets():
    try:
        db.session.rollback()
        # جلب الموردين لعرض محافظهم وأرصدتهم في "سوقك الذكي"
